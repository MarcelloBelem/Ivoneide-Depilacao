  ...
  return (
    ...
    >
      ...
      <div className="m-2.5 flex flex-col gap-2.5 border-t border-b py-2.5">
        ...
        <div className="flex flex-col gap-1.5">
          {appointment.services.map((service) => (
            <div
              key={service.id}
              className="font-lato flex justify-between px-2.5 text-base"
            >
              <p>{service.name}</p>
              <p>
      ...
      <div className="font-lato m-2.5 flex justify-around text-lg font-medium text-white">
        <button
          className="bg-error rounded-sm px-2.5 hover:bg-red-400"
          onClick={onReject}
        >
          Rejeitar
        </button>
        <button
          className="bg-success rounded-sm px-2.5 hover:bg-green-400"
          onClick={onAccept}
        >
          Aceitar
        </button>
